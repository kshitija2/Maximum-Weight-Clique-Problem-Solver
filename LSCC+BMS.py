...
#This program implements LSCC+BMS algorithm.
...
# This section greedily finds the clique with maximum weight
 ...
 # This section tries to find solution by using SCC and BMS both strategies
                ...
                if count==(len(cLocalBest)-1):
                    if i not in cLocalBest:
                        if[each,i] not in swapSet:
                            ...
                            v=age.get(i)+1
                            age[i]=v
#This section is different than in the program of LSCC algorithm. Following is the implementation of BMS heuristic.
        if swapSet:
            swapPair=[]
            swapPair1=[]
            swapPair2=[]
            ...
            for i in range(len(swapSet)):
                swapPair1=random.choice(swapSet)
                if confChange.get(swapPair1[1])==1:
                    break
                else:
                    continue
            swapOptimal=swapPair1

            for k in range(5):
                for i in range(len(swapSet)):
                    swapPair2=random.choice(swapSet)
                    if confChange.get(swapPair2[1])==1 and swapPair2!=swapPair:
                        break
                    else:
                        continue
                p=sumWeights-swapPair2[0]+swapPair2[1]
                q=sumWeights-swapOptimal[0]+swapOptimal[1]
                if p>q:
                    swapOptimal=swapPair2

            swapPair=swapOptimal
